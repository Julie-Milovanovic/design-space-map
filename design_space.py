...
def create_graph_FO(data):
    ...
    graph.opts(tools=['hover','wheel_zoom','reset'], active_tools=['wheel_zoom'], 
           node_size=4, cmap = 'blues', node_color='occurence', node_line_color='lightgrey',
           edge_color='lightgrey', edge_line_width = 0.5)
...
# definition for dynamic graph
def graph_dynamic_basic(time):
    grah_select = graph_bundled.select(time=time)
    grah_select.opts(node_size=10,node_line_color='lightseagreen',
           edge_color='lightseagreen', edge_line_width = 1)
    labels = hv.Labels(grah_select.nodes, ['x', 'y'], 'node')
    labels.opts(text_font_size='8pt', text_color='black', bgcolor='white',yoffset=0.025,  text_align='left')
    return (graph_bundled*grah_select*labels)

# ////////////////////////////////  FILE UPLOAD NLP  //////////////////////////////////////

session_01 = pd.read_csv(https://github.com/Julie-Milovanovic/design-space-map/blob/6fc51ac7d63f56ecadd86888b63d18a1f373f664/assets/session01.csv)
session_02 = pd.read_csv(https://github.com/Julie-Milovanovic/design-space-map/blob/6fc51ac7d63f56ecadd86888b63d18a1f373f664/assets/session02.csv)


# ////////////////////////////////  DISPLAY ON PAGE  //////////////////////////////////////

# Organization page
st.title("Representation of design space")
st.markdown("Tick the box on the side panel to analyze the design protocol.")
...
st.sidebar.markdown("Use this panel to analyze the data")

# //////////////////////////////// NLP / GRAPH ANALYSIS  //////////////////////////////////////

# NLP analysis / Graph display
st.title("Concept Analysis")
st.markdown("Tick the box on the side panel to represent design space.")
st.sidebar.subheader("Concept Analysis")

# Display graph
if.st.sidebar.checkbox("Process"):
    st.subheader("Concept graph evolution - Time")

# Display graph
if st.sidebar.checkbox("Time"):
	st.subheader("Concept graph evolution - Time")
...
# Plot graph
time_range=[*range(1,4,1)]
graph_bundled = create_graph_FO(session_01)
    
plots = {t:graph_dynamic_basic(t) for t in time_range}
    
@st.cache
def get_html():
    hmap = hv.HoloMap(plots, kdims=['Time'])
